      ...
      <template v-for="item in navItems" :key="item.name">
        ...
        >
          <div class="mr-4 w-5 h-5 flex items-center justify-center transition-transform duration-300 group-hover:scale-110 group-hover:text-ror-accent"
            ...
            <svg v-else-if="item.icon === 'backdoor'" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg>
          </div>
          <span class="font-medium tracking-wide">{{ item.name }}</span>
        </router-link>
      </template>
  ...
  if (session) {
    ...
    if (profile && profile.is_admin) {
      if (!navItems.value.find(item => item.path === '/dashboard/admin')) {
        navItems.value.push({ name: '後台管理', path: '/dashboard/admin', icon: 'backdoor' })
      }
    }
  }
